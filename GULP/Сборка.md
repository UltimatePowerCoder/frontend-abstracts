...
Я столкнулся с проблемой, когда использовал настройки сервера описанные выше, терминал выдает ошибку:
...
***Важно!** При подключении данного таска, нарушается работа плагина ```gulp-sourcemaps``` добавляющего актуальные пути для стилей!*